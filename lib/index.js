    ...
    bookshelf.Model = bookshelf.Model.extend({
        ...
        getList: function (options, columns) {
            return new Promise((resolve, reject) => {
              this.getListQuery(options, columns)
                  .then(q => q[`${options.limit}` === '-1' ? 'fetchAll' : 'fetchPage'](_.extend({
                    pageSize: options.limit || 10,
                    page: options.page || 1,
                    columns: (options.select && _.isString(options.select) ? options.select.split(',') : options.select) || undefined,
        ...
        getListQuery: function (options, columns) {
            return new Promise((resolve, reject) => {
                ...
                options = options || {};
                options.order_by = options.order_by || 'id';
                const q = this
                    .query(qb => this.toSearch(options, columns, qb));
                if (options.order_by) {
                  q.orderBy(options.order_by);
                }
                resolve(q);
            });
        },
        ...
        updateOne: function (options, updateColumns) {
            return new Promise((resolve, reject) => {
                ...
                const findByKeys = options.findByCompositePKey ? this.compositePKey : options.findByKeys || ['id'];
