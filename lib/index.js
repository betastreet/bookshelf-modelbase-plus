...
module.exports = (bookshelf) => {
    ...
    bookshelf.Model = bookshelf.Model.extend({
        ...
        bulkSync: function(existingCollection, desiredRows, columns, {noInsert, noUpdate, noDestroy} = {}) {
            ...
            results.destroyed = noDestroy ? [] : existingCollection.serialize();

            return bookshelf.transaction((transacting) =>
              Promise.all([
                  !noInsert && results.inserted.length ? this.bulkInsert(results.inserted, { transacting, returnInserted: true }) : [],
                  !noUpdate && results.updated.length ? results.updated.invokeThen('save', null, { transacting }) : [],
                  !noDestroy && existingCollection.length ? existingCollection.invokeThen('destroy', { transacting }) : [],
                ]))
              .spread((inserted, updated, destroyed) => {
                results.inserted = inserted;
                results.updated = updated.map(u => u.serialize());
                return results;
              });
        },
        ...
        bulkUpdate: function(rows, columns = [], { transacting, returnInserted } = {}) {
          ...
          return Promise
            ...
            .then(rows => columns.length ? _.map(rows, row => _.pick(row, columns)) : rows)
            ...
            .then((rows) => {
              const updateColArr = columns || [];
              if (!updateColArr.length) {
