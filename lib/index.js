...
module.exports = (bookshelf) => {
    ...
    bookshelf.Model = bookshelf.Model.extend({
        ...
        toSearch: function(options, columns, searchChain) {
          ...
          _.forEach(whereVals, (v, k) => whereVals[k] = this.sanitizeVal(v));
          const search = _.reduce(filterNested, (chain, item, key) => {
            ...
            if (operator === 'BETWEEN') {
              return chain.andWhereBetween(key, value);
            } else if (operator === 'NOT_BETWEEN') {
              return chain.andWhereNotBetween(key, value);
            } else {
              return chain.andWhere(key, operator, value);
            }
