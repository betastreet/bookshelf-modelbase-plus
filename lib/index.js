...
module.exports = (bookshelf) => {
    ...
    bookshelf.Model = bookshelf.Model.extend({
        ...
        bulkInsert: function(rows, { transacting, returnInserted } = {}) {
            ...
            return knext(bookshelf.knex, model.tableName, transacting)
                .insert(data)
                .then(() => {
                    if (returnInserted) {
                        return knext(bookshelf.knex, model.tableName, transacting)
                            ...
                            .then(binaryRows => binaryRows.map(r => transformBinarytoPOUUID(bookshelf, model, r)))
                    }
                    return rows.length;
                });
        },
        ...
        bulkUpdate: function(rows, { transacting, returnInserted, columns } = {}) {
            if (!Array.isArray(rows)) {
                return Promise.reject(new Error('`rows` is not an array'));
            }
            const model = new this();
            ...
            function validateRows(rows) {
                ...
                const validationErrors = rows
                    .map((row) => {
                        try {
                            const rowToValidate = _.pickBy(row, (v,k) => {
                                if (columns) {
                            ...
                            const mod = this.forge(rowToValidate);
                            mod.validateSave(mod, rowToValidate, { method: 'insert', patch: false });
                            return { error: null };
                        } catch (validationError) {
                            return { error: validationError };
                        }
            ...
            return knext(bookshelf.knex, model.tableName, transacting)
                .select()
                .whereIn(idAttribute, _.map(data, idAttribute))
                .then(existingRows => existingRows.map(e => transformBinarytoPOUUID(bookshelf, model, e)))
                ...
                .tap(updatedRowsNotSaved => validateRows.call(this, updatedRowsNotSaved))
                .then(rows => rows.map(row => transformPOUUIDtoBinary(bookshelf, model, row)))
                .then((rows) => {
                    let updateColArr = [];
                    ...
                    let query = knext(bookshelf.knex, model.tableName, transacting).insert(rows).toString();
                ...
                .then((queryStats) => {
                    if (returnInserted) {
                        return knext(bookshelf.knex, model.tableName)
                        .select()
                        .whereIn(idAttribute, _.map(data, idAttribute))
                        .then(binaryRows => binaryRows.map(r => transformBinarytoPOUUID(bookshelf, model, r)));
                    } else {
...
function transformBinarytoPOUUID(bookshelf, model, row) {
    if (_.isEmpty(model.orderedUuids)) {
        return row;
    }
...
function knext(knex, tableName, transacting = null) {
    return transacting ? knex(tableName).transacting(transacting) : knex(tableName);
}
