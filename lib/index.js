        ...
        getList: function (options, columns) {
            return new Promise((resolve, reject) => {
              this.getListQuery(options, columns)
                  .then((q) => {
                    const noLimit = `${options.limit}` === '-1';
                    const noPagination = !_.isNil(options.paginate) && (!options.paginate || options.paginate === '0');
                    if (noPagination && !noLimit) {
                      q = q.query((qb) => {
                        const limit = options.limit || 10;
                        return qb.limit(limit).offset(limit * ((options.page || 1) - 1));
                      });
                    }
                    return q[(noLimit || noPagination) ? 'fetchAll' : 'fetchPage'](_.extend({
