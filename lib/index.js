...
module.exports = (bookshelf) => {
    ...
    bookshelf.Model = bookshelf.Model.extend({
        ...
        bulkUpdate: function(rows, { transacting, returnInserted } = {}) {
            if (!Array.isArray(rows)) {
                return Promise.reject(new Error('`rows` is not an array'));
            }
            const model = new this();
            ...
            // read existing rows
            bookshelf.knex(model.tableName)
                .select()
                .whereIn(idAttribute, _.map(data, idAttribute))
                .tap(existingRows => console.log('existing rows', existingRows))
                .then(existingRows => {
                    existingRows.forEach(e => {
                        let id = e[`${idAttribute}`];
                        if (Buffer.isBuffer(id)) {
                            id = bookshelf.Model.binaryToPrefixedUuid(id, 2);
                        }
                        const findById = _.find(rows, { [idAttribute]: id });
                        Object.assign(e, findById);
                    });
                    return existingRows;
                })
                .then(rows => rows.map(row => transformPOUUIDtoBinary(bookshelf, model, row)))
                .tap(rows => console.log('update2', rows))
                .tap((rows) => {
                    // fixme
                    return;
                    if (model.validate && model.validateSave) {
                ...
                .then((rows) => {
                    ...
                    let query = bookshelf.knex(model.tableName).insert(rows).toString();
