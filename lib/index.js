...
module.exports = (bookshelf) => {
    ...
    bookshelf.Model = bookshelf.Model.extend({
        ...
        bulkSync: function(existingCollection, desiredRows, columns, {noInsert, noUpdate, noDestroy} = {}) {
            const model = new this();
            if (!(existingCollection instanceof model.Collection)) {
              return Promise.reject('must pass an existing collection');
            }
            ...
            _.forEach(desiredRows, (item) => {
                const existing = existingCollection.find({[idAttribute]: item[idAttribute]});
                if (!existing) {
                    ...
                    results[needsUpdate ? 'updated' : 'unchanged'].push(existing.serialize());
            ...
            return bookshelf.transaction((transacting) =>
              Promise.all([
                  ...
                  !noUpdate && results.updated.length ? this.bulkUpdate(results.updated, cols, { transacting, returnInserted: true, skipRead: true }) : [],
         ...
         */
        bulkUpdate: function(rows, columns = [], { transacting, returnInserted, skipRead } = {}) {
          if (!Array.isArray(rows)) {
            return Promise.reject(new Error('`rows` is not an array'));
          }
          const model = new this();
          ...
          return Promise
            .resolve()
            .then(() => skipRead ? rows : this
              .knext(transacting)
              .select()
              .whereIn(idAttribute, _.map(data, idAttribute))
            )
            .tap(found => prevRows = found)
            .then(existingRows => existingRows.map(e => this.transformBinaryToPouuid(e)))
            .then(existingRows => !skipRead ? existingRows : rows.map(rowWithUpdates => {
              ...
              const merged = Object.assign(found, rowWithUpdates);
              return columns.length ? _.pick(merged, columns) : merged;
            }))
            ...
            .then((rows) => {
              const updateColArr = Object.keys(rows[0]).map(e => `${e}=values(${e})`);
                ...
                if (returnInserted) {
                  return this
                    ...
                    .map(r => {
                      ...
                      const found = _.find(prevRows, {[idAttribute]: item[idAttribute]});
                      if (found) {
                        item._savedAttributes = found;
                      }
                      return item;
                    })
                } else {
